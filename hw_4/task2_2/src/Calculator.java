...
public class Calculator {
    ...
    public static void main(String[] arg) {
        ...
        String sign;
        float operand1=1, operand2=1, rez = 0.F;
        boolean flag = true;

        System.out.print("консульний калькулятор%n");
        System.out.print("Введить 1 числo: ");
        if (inStr.hasNextFloat()) {
            operand1 = (float) inStr.nextDouble();
            if (inStr.hasNextFloat()) {
                operand2 = (float) inStr.nextDouble();
            }
        }
        ...
        switch (sign) {
            ...
            default -> {
                System.out.printf("%sНе правільно введена операція - %s %s", YELLOW_BOLD, sign, ANSI_RESET);
                flag = false;
            }
        }
        if (flag)
